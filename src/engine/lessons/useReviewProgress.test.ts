...
// This test env has no VITE_FIREBASE_* config, so firebaseReady is false and
// the hook must degrade gracefully — same contract as useLessonProgress.
function wrapper({ children }: { children: React.ReactNode }) {
  return createElement(AuthProvider, null, children);
}

describe('useReviewProgress (no Firebase configured)', () => {
  ...
  it('recordResult resolves without throwing when Firebase is unavailable', async () => {
    const { result } = renderHook(() => useReviewProgress(), { wrapper });
